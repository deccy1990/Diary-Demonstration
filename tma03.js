 ...
 */
/** Comment out the entire function to disable adding demonstration entries
...
function addTextEntry(key, initialText, isNewEntry) {
    ...
    function saveEntry() {
        ...
        function saveEntry() {
            ...
            var textValue = textareaElement.value;

            // Make the text item using the value
            var textItem = makeItem("text", textValue);

            // Store the item in local storage using the given key
            localStorage.setItem(key, textItem);
        }
...
function addImageEntry(key, data) {
    ...
    // Store the item in local storage using the given key
    var imageItem = makeItem("image", data);
...
function processFile(inputChangeEvent) {
    ...
    // Create a FileReader to read the file into a data URL
    var reader = new FileReader();

    // Define the function to handle the FileReader 'load' event
    reader.onload = function (loadEvent) {
        console.log("FileReader onload called with arguments:", {
            loadEvent,
        });
        ...
        // Comment out the line below using window.DUMMY_DATA_URL
        // Add a 'dummy' image entry
        // addImage(window.DUMMY_DATA_URL);

        // Call addImageEntry using the data URL you obtain
        addImageEntry("diary" + Date.now(), dataURL);
    };

    // Read the selected file as a data URL
    reader.readAsDataURL(inputChangeEvent.target.files[0]);
